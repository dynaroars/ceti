...
namespace crest{
  ...
  void SymExpr::AppendVars(set<var_t> *vars) const{
      for (auto c: coeff_){
	vars->insert(c.first);
      }
    }

  bool SymExpr::DependsOn(const map<var_t, type_t> &vars) const{
      for(auto c:coeff_){
  ...
  /*** SymPred ***/
  SymPred::SymPred()
    : op_(ops::EQ), expr_(new SymExpr(0)){}
  ...
  const string SymPath::str() const{
    ...
    ss << "(path) " << "branches " << container2str(branches_) << endl;
    ss << "constraints " << endl ;
    ss << constraints2str(constraints_) << endl;
    ss << "constraints_idx " << container2str(constraints_idx_);
    return ss.str();
  }
  ...
  bool SymPath::Parse(std::istream &s){
    ...
    //cout << "constraints " << container2str(constraints_) << endl;
  ...
  const string SymExec::str() const{
    ...
    ss << "(exec) "
       << "vars " << map2str(vars()) << ", "
       << "inputs " << container2str(inputs_) << endl;
  ...
  string constraints2str(const vector<SymPred *> &constraints){
    stringstream ss;
  ...
  string constraints2str(const vector<const SymPred *> &constraints){
    stringstream ss;
    for(auto cst: constraints){
      ss << cst->str() << endl;
    }
